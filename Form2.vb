...
Public Class Form2
    ...
    Private Sub Form2_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            conexsql = New SqlConnection(CONEXION_GASTOS)
            conexsql.Open()
            sql = "SELECT * FROM (SELECT CLAVE, DESCRIP FROM VARIOS UNION ALL SELECT 0,'SELECCIONE ......') D1 ORDER BY D1.DESCRIP"
            sqlda = New SqlDataAdapter(sql, conexsql)
    ...
    Private Sub txtOperador_Leave(sender As Object, e As EventArgs) Handles txtOperador.Leave
        Try
            ...
            sql = "SELECT id_personal No_Operador,nombre FROM personal_personal WHERE id_categoria = 29 AND estado = 'A' AND id_personal = " & txtOperador.Text
    ...
    Private Sub txtequipo_TextChanged(sender As Object, e As EventArgs) Handles txtequipo.TextChanged
    ...
    Private Sub txtequipo_MouseLeave(sender As Object, e As EventArgs) Handles txtequipo.MouseLeave

    End Sub

    Private Sub txtTalon1_LocationChanged(sender As Object, e As EventArgs) Handles txtTalon1.LocationChanged

    End Sub

    Private Sub txtequipo_Leave(sender As Object, e As EventArgs) Handles txtequipo.Leave

        Dim sql_txtequipo = ""
        Dim SQLDR_TXTEQUIPO As SqlDataReader
        conexsql = New SqlConnection(CONEXION_DB)
        conexsql.Open()
        sql_txtequipo = "SELECT  id_unidad FROM  mtto_unidades WHERE id_tipo_unidad in (1, 5, 10, 14) AND tercero = 'N' AND id_unidad like '%" & txtTalon8.Text & "%'"

        Dim SQLCOM_TXTEQUIPO = New SqlCommand
        SQLCOM_TXTEQUIPO.CommandText = sql_txtequipo
        SQLCOM_TXTEQUIPO.Connection = conexsql
        SQLDR_TXTEQUIPO = SQLCOM_TXTEQUIPO.ExecuteReader
        If Not SQLDR_TXTEQUIPO.Read Then
            MsgBox("NO ES UN EQUIPO VALIDO")

        End If

    End Sub

    Private Sub txtcausa8_Leave(sender As Object, e As EventArgs) Handles txtcausa8.Leave
